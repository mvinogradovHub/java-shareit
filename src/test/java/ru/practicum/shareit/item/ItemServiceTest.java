...
class ItemServiceTest {
  ...
  @Captor private ArgumentCaptor<Item> itemArgumentCaptor;
  ...
  void init() {
    ...
    BookingWithoutObjDto bookingDto =
    ...
    when(itemRepository.save(Mockito.any()))
        .thenReturn(
            ItemMapper.itemDtoToItem(
                itemDto, user, ItemRequestMapper.itemRequestToItemRequestDto(itemRequest)));
  ...
  void updateItem_whenUserNotFound_thenNotFoundException() {
    ...
    assertThrows(
        ...
        () -> itemService.updateItem(itemDto, user.getId(), itemDto.getId()));
    verify(itemRepository, never()).save(ItemMapper.itemDtoToItem(itemDto, user, null));
  }
  ...
  void updateItem_whenUserNotHaveRightsToEdit_thenNotFoundException() {
    ...
    assertThrows(
        NotFoundException.class, () -> itemService.updateItem(itemDto, 99L, itemDto.getId()));
    verify(itemRepository, never()).save(ItemMapper.itemDtoToItem(itemDto, user, null));
  }
  ...
  void updateItem_whenUserAndItemFound_thenUpdateItem() {
    ...
    ItemDto newItemDto =
        ItemDto.builder()
            ...
            .owner(UserDto.builder().id(33L).build())
            .available(false)
            .requestId(6L)
            .build();
  ...
  void getItem_whenInvoke_returnedItemDto() {
    when(userRepository.findById(Mockito.anyLong())).thenReturn(Optional.of(user));
    when(itemRepository.findById(Mockito.anyLong())).thenReturn(Optional.of(item));
    when(bookingRepository.getItemLastBooking(Mockito.anyLong(), Mockito.any()))
        .thenReturn(Optional.of(booking));
    when(bookingRepository.getItemNextBooking(Mockito.anyLong(), Mockito.any()))
        .thenReturn(Optional.of(booking));
  ...
  void searchItems_whenUserNotFound_thenNotFoundException() {
    ...
    assertThrows(
        NotFoundException.class, () -> itemService.searchItems(user.getId(), "текст", 0, 10));
    verify(itemRepository, never()).searchItems(Mockito.any(), Mockito.any());
  }
  ...
  void addComment_whenInvoke_thenReturnedCommentDto() {
    when(userRepository.findById(Mockito.anyLong())).thenReturn(Optional.of(user));
    when(itemRepository.findById(Mockito.anyLong())).thenReturn(Optional.of(item));
    when(bookingRepository.getByBookerAndItemPastApprovedBooking(
    ...
    CommentDto commentDto =
        itemService.addComment(
            CommentMapper.commentToCommentDto(comment), user.getId(), item.getId());
