...
class BookingValidatorTest {
    ...
    void checkItemOwnerOrAfter_whenUserNotOwnerOrAfter_thenNotFoundException() {
        assertThrows(NotFoundException.class, () -> bookingValidator.checkItemOwnerOrAfter(booking, 99L));
    }

    @Test
    void checkItemOwnerOrAfter_whenUserOwnerOrAfter_thenNotException() {
        assertDoesNotThrow(() -> bookingValidator.checkItemOwnerOrAfter(booking, 1L));
    }
    ...
    void checkItemAvailable_whenItemAvailable_thenNotException() {
        assertDoesNotThrow(() -> bookingValidator.checkItemAvailable(item));
    }

    @Test
    void checkBookingStartBeforeEnd_whenStartAfterEnd_thenBadDataException() {
        assertThrows(BadDataException.class, () -> bookingValidator.checkBookingStartBeforeEnd(booking));
    }

    @Test
    void checkBookingStartBeforeEnd_whenStartBeforeEnd_thenNotException() {
        booking.setEnd(LocalDateTime.of(2023, 5, 10, 0, 0, 0, 0));
        assertDoesNotThrow(() -> bookingValidator.checkBookingStartBeforeEnd(booking));
    }
    ...
    void checkNotYourOwnItem_WhenUserOwnerItem_thenNotFoundException() {
        assertThrows(NotFoundException.class, () -> bookingValidator.checkNotYourOwnItem(user.getId(), booking));
    }
