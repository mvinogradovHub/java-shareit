...
class BookingControllerTest {
        ...
        mockMvc
            .perform(
                post("/bookings")
                    ...
                    .content(objectMapper.writeValueAsString(bookingWithoutObjDto)))
  ...
  void getBooking_whenInvoke_thenReturnBookingDto() {
    ...
    String result =
        mockMvc
            .perform(get("/bookings/{bookingId}", 1L).header("X-Sharer-User-Id", 1L))
            .andExpect(status().isOk())
            .andReturn()
            .getResponse()
            .getContentAsString();
  ...
  void getBookerBookings_whenInvokeWithStateNull_thenInvokeGetBookerBookingsWithStateAll() {
    ...
    mockMvc
        .perform(get("/bookings").header("X-Sharer-User-Id", 1L).param("size", "10"))
        .andExpect(status().isOk())
        .andReturn()
        .getResponse()
        .getContentAsString();
  ...
  void getBookerBookings_whenInvokeWithFromNull_thenInvokeGetBookerBookingsWithFromZero() {
    ...
    mockMvc
        .perform(
            get("/bookings").header("X-Sharer-User-Id", 1L).param("from", "13").param("size", "10"))
        .andExpect(status().isOk())
        .andReturn()
        .getResponse()
        .getContentAsString();
  ...
  void getOwnerBookings_whenInvokeWithStateNull_thenInvokeGetBookerBookingsWithStateAll() {
    ...
    mockMvc
        .perform(get("/bookings/owner").header("X-Sharer-User-Id", 1L).param("size", "10"))
        .andExpect(status().isOk())
        .andReturn()
        .getResponse()
        .getContentAsString();
